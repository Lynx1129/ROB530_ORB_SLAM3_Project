...
# Dataset website: http:/python -m pip install scipypython -m pip install scipy/theairlab.org/tartanair-dataset/
...
def get_trajectory_list(envname, easy_hard='Easy'):
    ...
    traj_gen = container_client.walk_blobs(
...
print('Find {} trajectories in {}'.format(
    len(trajlist), envlist[env_ind]+diff_level))
print(trajlist)
...
# try to process one optical flow
...
fx = np.mean(flow[..., 0][flow_mask > 0])
fy = np.mean(flow[..., 1][flow_mask > 0])
